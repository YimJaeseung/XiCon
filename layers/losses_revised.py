...
class AutoConCI_revised(nn.Module):  # AutoCon Channel Independence (CI) version for Multivariate
    def __init__(self, batch_size, seq_len, acf_values, xicor_values, temperature=0.07, contrast_mode='all',
                 base_temperature=0.07, omega=0.5):
        super(AutoConCI_revised, self).__init__()
        self.batch_size = batch_size
        self.temperature = temperature
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
    ...
    def local_contrastive_loss(self, features, labels):
        ...
        mean_log_local_prob_pos = (local_distmap * pos_mask * log_local_prob).sum(2) / pos_mask.sum(2)
        """
        #mean_log_local_prob_pos = (local_map * pos_mask * log_local_prob).sum(2) / pos_mask.sum(2)
    ...
    def avg_global_contrastive_loss(self, features, labels):
        ...
        #mean_log_global_prob_pos = (global_map * pos_mask * log_global_prob).sum(2) / pos_mask.sum(2)
