...
class AutoCon(nn.Module):
    def __init__(self, batch_size, seq_len, acf_values, xicor_values, temperature=0.07, contrast_mode='all',
        ...
        self.xicor_values = torch.from_numpy(xicor_values)
...
class XiCon(nn.Module):  # XiCon Channel Independence (CI) version for Multivariate
    def __init__(self, batch_size, seq_len, acf_values, xicor_values, patch_len, stride, temperature=0.07, contrast_mode='all',
                 base_temperature=0.07, omega=0.5, patching=False):
        super(XiCon, self).__init__()
        self.batch_size = batch_size
        self.temperature = temperature
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
    ...
    def local_contrastive_loss(self, features, labels):
        ...
        local_ximap = (labels.unsqueeze(1) - labels.unsqueeze(2)).abs()  # (BC, T, T) ,  (C, L)
        local_ximap = local_ximap.reshape(self.batch_size, -1, T*T).cpu()
        xicor_values = self.xicor_values.unsqueeze(0).repeat(self.batch_size,1, 1)
        local_ximap = torch.gather(xicor_values, 2, local_ximap).float().to(features.get_device())
        local_ximap = local_ximap.reshape(-1, T, T)


        local_map = self.omega * local_distmap + (1 - self.omega) * local_ximap


        neg_mask = torch.scatter(
            torch.ones_like(local_map).to(features.get_device()),
            2,
            torch.arange(T).reshape(1, -1, 1).repeat(BC, 1, 1).to(features.get_device()),
            0
        )
        ...
        self_mask = (local_distmap == 1.0)
        pos_mask = local_autocorr_mask(local_map, self_mask) + (neg_mask * self_mask)

        exp_local_logits = torch.exp(local_logits) * neg_mask  # denominator

        log_local_prob = local_logits - torch.log(exp_local_logits.sum(2, keepdim=True) + epsilon)  # (B, T, T) > (B, T ,1)

        mean_log_local_prob_pos = (local_map * pos_mask * log_local_prob).sum(2) / (pos_mask.sum(2) + epsilon)
    ...
    def avg_global_contrastive_loss(self, features, labels):
        ...
        global_ximap = (labels.unsqueeze(0) - labels.unsqueeze(1)).abs()  #(B, B)
        global_ximap = global_ximap.reshape(1, B * B).repeat(C, 1).cpu()
        global_ximap = torch.gather(self.xicor_values,1, global_ximap).float().to(device)
        global_ximap = global_ximap.reshape(C, B, B)

        
        global_map = self.omega * global_distmap + (1 - self.omega) * global_ximap


        neg_mask = torch.scatter(
            torch.ones_like(global_map),
            2,
            torch.arange(B).reshape(1, -1, 1).repeat(C, 1, 1).to(device),
            0
        )
        ...
        pos_mask = autocorr_mask_with_CI(global_map, self_mask) + (neg_mask * self_mask)

        exp_global_logits = torch.exp(global_logits) * neg_mask  # denominator

        
        log_global_prob = global_logits - torch.log(exp_global_logits.sum(2, keepdim=True) + epsilon)  # (C, B, B) > (C, B ,1)


        mean_log_global_prob_pos = (global_map * pos_mask * log_global_prob).sum(2) / (pos_mask.sum(2) + epsilon)
    ...
    def forward(self, features, labels=None):
        ...
        B, I, D = features.shape
        if self.patching == True:
            feature_idxs = torch.rand(B, self.patch_num).argsort(-1)[:, :self.patch_num//3].to(features.get_device())
        else:
            feature_idxs = torch.rand(B, self.seq_len).argsort(-1)[:, :self.seq_len//3].to(features.get_device())
        selected_features = torch.gather(features, 1, feature_idxs.unsqueeze(-1).repeat(1, 1, D))
...
if __name__ == '__main__':
    ...
    xicor_values = np.array([1.0, 0.3, 0.1, -0.3, 0.2, 0.4, 0.1, 0.0])
    print(acf_values[distmap])
    supcreg = XiCon(acf_values, xicor_values, contrast_mode='all')#.cuda()
