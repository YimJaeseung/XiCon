...
from layers.losses import AutoCon, AutoConCI, XiCon
...
class Exp_Long_Term_Forecast_with_AutoCon(Exp_Basic):
    ...
    def init_AutoCon(self, args):
        ...
        elif self.args.model == 'AutoCon_PatchMixer':
            acf_values = np.stack(acf_values, axis=0)
            loss = AutoConCI(args.batch_size, args.seq_len, np.abs(acf_values), temperature=1.0, base_temperature=1.0)
            print(f'Auto-correlation values(abs):{acf_values[0, :2]} ~ {acf_values[0, -2:]}')
        elif self.args.model == 'AutoCon_PatchMixer_1dconv':
            acf_values = np.stack(acf_values, axis=0)
            loss = AutoConCI(args.batch_size, args.seq_len, np.abs(acf_values), temperature=1.0, base_temperature=1.0)
            print(f'Auto-correlation values(abs):{acf_values[0, :2]} ~ {acf_values[0, -2:]}')
        elif self.args.model == 'AutoCon_PatchMixer_loess':
            acf_values = np.stack(acf_values, axis=0)
            loss = AutoConCI(args.batch_size, args.seq_len, np.abs(acf_values), temperature=1.0, base_temperature=1.0)
            print(f'Auto-correlation values(abs):{acf_values[0, :2]} ~ {acf_values[0, -2:]}')
        elif self.args.model == 'AutoCon_PatchMixer_longshort':
            acf_values = np.stack(acf_values, axis=0)
            loss = AutoConCI(args.batch_size, args.seq_len, np.abs(acf_values), temperature=1.0, base_temperature=1.0)
            print(f'Auto-correlation values(abs):{acf_values[0, :2]} ~ {acf_values[0, -2:]}')
        elif self.args.model == 'AutoCon_PatchMixer_longshort_PMDv2':
            acf_values = np.stack(acf_values, axis=0)
            loss = AutoConCI(args.batch_size, args.seq_len, np.abs(acf_values), temperature=1.0, base_temperature=1.0)
            print(f'Auto-correlation values(abs):{acf_values[0, :2]} ~ {acf_values[0, -2:]}')
        elif self.args.model == 'AutoCon_PatchMixer_longtermonly':
            acf_values = np.stack(acf_values, axis=0)
            loss = AutoConCI(args.batch_size, args.seq_len, np.abs(acf_values), temperature=1.0, base_temperature=1.0)
            print(f'Auto-correlation values(abs):{acf_values[0, :2]} ~ {acf_values[0, -2:]}')
        elif self.args.model == 'AutoCon_PatchMixer_longtermonly_PMD':
            acf_values = np.stack(acf_values, axis=0)
            loss = AutoConCI(args.batch_size, args.seq_len, np.abs(acf_values), temperature=1.0, base_temperature=1.0)
            print(f'Auto-correlation values(abs):{acf_values[0, :2]} ~ {acf_values[0, -2:]}')
        else:
